...
import { Package, Lock, AlertCircle, CheckCircle2, RefreshCw, Search, ShieldCheck, ArrowDownRight, Layers } from 'lucide-react';
      ...
      <div className="glass-panel rounded-2xl p-6 border border-slate-800/80 shadow-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-xl font-bold text-white tracking-tight">Bảng Cân Đối Tồn Kho (Real-Time Stock Balance)</h1>
              <p className="text-xs text-slate-400">
                Lõi phân bổ: <span className="text-white font-mono font-bold">On-Hand</span> (Vật lý) = <span className="text-amber-400 font-mono font-bold">Reserved</span> (Giữ đơn) + <span className="text-emerald-400 font-mono font-bold">Available</span> (Khả dụng)
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            ...
            <input
              type="text"
              placeholder="Tìm SKU, tên hàng, ô kệ..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-8 pr-3 py-1.5 bg-slate-900 border border-slate-700/60 rounded-xl text-xs text-slate-200 placeholder-slate-400 focus:outline-none focus:border-cyan-500"
            />
          </div>
